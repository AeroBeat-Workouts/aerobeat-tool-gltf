...
func load_scene_bundle(request: Dictionary) -> Dictionary:
	...
	var normalized_path: String = _normalize_asset_path(asset_path)
	var resource_path: String = _resource_path_for(asset_path)
...
func load_scene_instance_bundle(request: Dictionary) -> Dictionary:
	...
	return {
		"ok": true,
		"asset_path": asset_path,
		"absolute_path": _normalize_asset_path(asset_path),
		"resource_path": _resource_path_for(asset_path),
		"format": format,
		"container": String(request.get("container", format)),
...
func load_scene_bundles(request: Dictionary) -> Dictionary:
	...
	for instance_variant in _array_or_empty(request.get("instances", [])):
		...
		vendor_instances.append({
			...
			"source": {
				"path": String(instance_request.get("asset_path", "")).strip_edges(),
				"format": String(instance_request.get("format", instance_request.get("container", ""))).strip_edges().to_lower(),
			},
	...
	for index in range(vendor_results.size()):
		...
		normalized_instances.append({
			"asset_path": String(requested_instance.get("asset_path", "")),
			"absolute_path": _normalize_asset_path(String(requested_instance.get("asset_path", ""))),
			"resource_path": _resource_path_for(String(requested_instance.get("asset_path", ""))),
			"format": String(requested_instance.get("format", requested_instance.get("container", ""))),
			"container": String(requested_instance.get("container", requested_instance.get("format", ""))),
